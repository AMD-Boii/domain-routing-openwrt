...
check_repo() {
    ...
    # REVIEW не знаю, какое сообщение apk дает при подобной ошибке
...
dnsmasqfull() {
    if apk list --installed | grep -q dnsmasq-full; then
        ...
        # REVIEW Не понимаю, при каких условиях может появится dhcp-opkg, но все равно поменял на apk для общего вида
...
install_awg_packages() {
    # REVIEW Получение pkgarch с наибольшим приоритетом
    # Не уверен, что вывод этой apk команды дает ожидаемый результат.. Есть еще, кстати, uname -m
    PKGARCH=$(apk --print-arch)
